...
const AdminEducation = () => {
  ...
  useEffect(() => { api.get('/education').then(r => setData(r.data)).catch(() => showToast('Failed to load.')); }, []);

  const handleDelete = async (id) => {
    try { await api.delete(`/education/${id}`); setData(d => d.filter(s => s._id !== id)); setConfirmId(null); showToast('Deleted.', 'success'); }
    catch { showToast('Delete failed.'); }
  };

  const handleAdd = async (e) => {
    e.preventDefault();
    try { const res = await api.post('/education', form); setData(d => [...d, res.data]); setShowModal(false);
      setForm({ degree: '', institution: '', startDate: '', endDate: '', description: '', order: 0 }); showToast('Added.', 'success'); }
    ...
    <div className="min-h-screen p-6 md:p-8" style={{ background: '#0f0f13' }}>
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-wrap justify-between items-center gap-3 mb-8 animate-fade-in-up">
          ...
          <h2 className="text-xl font-bold text-white tracking-tight">Education Management</h2>
        ...
        <div className="glass rounded-2xl overflow-x-auto animate-fade-in-up" style={{ animationDelay: '80ms' }}>
          <table className="w-full text-left min-w-[600px]">
            <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              {['Degree / Certificate', 'Institution', 'Duration', 'Actions'].map((h, i) => (
            ...
            <tbody>
              ...
              {data.length === 0 && <tr><td colSpan={4} className="px-5 py-12 text-center text-slate-600 text-sm">No education yet. Add one above.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
          <div className="glass rounded-3xl w-full max-w-md overflow-hidden animate-slide-up">
            ...
            <form onSubmit={handleAdd} className="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Degree / Certificate</label>
                <input required type="text" placeholder="e.g. BS Computer Science, MBA, Photography Certificate" className={inputCls} style={inputStyle} value={form.degree} onFocus={focusStyle} onBlur={blurStyle} onChange={set('degree')} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Institution</label>
                <input required type="text" placeholder="e.g. MIT, Coursera, University of London" className={inputCls} style={inputStyle} value={form.institution} onFocus={focusStyle} onBlur={blurStyle} onChange={set('institution')} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Start Date</label>
                  <input type="text" placeholder="e.g. Sep 2018" className={inputCls} style={inputStyle} value={form.startDate} onFocus={focusStyle} onBlur={blurStyle} onChange={set('startDate')} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">End Date</label>
                  <input type="text" placeholder="e.g. Jun 2022, Present" className={inputCls} style={inputStyle} value={form.endDate} onFocus={focusStyle} onBlur={blurStyle} onChange={set('endDate')} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Description</label>
                <textarea rows={3} placeholder="e.g. Relevant coursework, achievements, GPA..." className={inputCls} style={inputStyle} value={form.description} onFocus={focusStyle} onBlur={blurStyle} onChange={set('description')} />
              </div>
