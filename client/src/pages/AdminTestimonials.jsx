...
import { Trash2, Plus, ArrowLeft, X, AlertTriangle, Star } from 'lucide-react';
import api from '../api';
...
const AdminTestimonials = () => {
  ...
  useEffect(() => { api.get('/testimonials').then(r => setData(r.data)).catch(() => showToast('Failed to load.')); }, []);

  const handleDelete = async (id) => {
    try { await api.delete(`/testimonials/${id}`); setData(d => d.filter(s => s._id !== id)); setConfirmId(null); showToast('Deleted.', 'success'); }
    catch { showToast('Delete failed.'); }
  };

  const handleAdd = async (e) => {
    e.preventDefault();
    try { const res = await api.post('/testimonials', form); setData(d => [...d, res.data]); setShowModal(false);
      setForm({ name: '', role: '', text: '', rating: 5, source: '', sourceUrl: '', order: 0 }); showToast('Added.', 'success'); }
    ...
    <div className="min-h-screen p-6 md:p-8" style={{ background: '#0f0f13' }}>
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-wrap justify-between items-center gap-3 mb-8 animate-fade-in-up">
          ...
          <h2 className="text-xl font-bold text-white tracking-tight">Testimonials Management</h2>
        ...
        <div className="glass rounded-2xl overflow-x-auto animate-fade-in-up" style={{ animationDelay: '80ms' }}>
          <table className="w-full text-left min-w-[600px]">
            <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              {['Client', 'Review', 'Rating', 'Actions'].map((h, i) => (
            ...
            <tbody>
              ...
              {data.length === 0 && <tr><td colSpan={4} className="px-5 py-12 text-center text-slate-600 text-sm">No testimonials yet. Add one above.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
          <div className="glass rounded-3xl w-full max-w-md overflow-hidden animate-slide-up">
            <div className="flex justify-between items-center px-6 py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 className="text-lg font-bold text-white">Add Testimonial</h3>
            ...
            <form onSubmit={handleAdd} className="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Client Name</label>
                <input required type="text" placeholder="e.g. Sarah Johnson, Ahmed Ali" className={inputCls} style={inputStyle} value={form.name} onFocus={focusStyle} onBlur={blurStyle} onChange={set('name')} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Client Role / Company</label>
                <input type="text" placeholder="e.g. CEO at TechCorp, Freelance Designer" className={inputCls} style={inputStyle} value={form.role} onFocus={focusStyle} onBlur={blurStyle} onChange={set('role')} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Review Text</label>
                <textarea required rows={3} placeholder="What did the client say about your work?" className={inputCls} style={inputStyle} value={form.text} onFocus={focusStyle} onBlur={blurStyle} onChange={set('text')} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Rating</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" placeholder="e.g. LinkedIn, Upwork, Fiverr, Google" className={inputCls} style={inputStyle} value={form.source} onFocus={focusStyle} onBlur={blurStyle} onChange={e => setForm(f => ({ ...f, source: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Review URL <span className="normal-case text-slate-600">(proof link)</span></label>
                  <input type="url" placeholder="https://linkedin.com/in/..." className={inputCls} style={inputStyle} value={form.sourceUrl} onFocus={focusStyle} onBlur={blurStyle} onChange={e => setForm(f => ({ ...f, sourceUrl: e.target.value }))} />
                </div>
              </div>
