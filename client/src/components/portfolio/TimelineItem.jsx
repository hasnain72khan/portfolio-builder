const TimelineItem = ({ title, subtitle, dateRange, description, dotColor, brandDot = true }) => (
  <div className="rounded-2xl p-5 relative pl-8" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
    <div className={`absolute left-5 top-7 w-2 h-2 rounded-full ${!brandDot && dotColor ? dotColor : ''}`}
      style={brandDot ? { background: 'var(--color-brand)' } : undefined} />
    <div className="flex items-start justify-between gap-4 mb-1">
      <div>
        <h3 className="font-bold text-sm" style={{ color: 'var(--color-heading)' }}>{title}</h3>
        <p className="text-xs font-medium" style={{ color: dotColor === 'bg-teal-400' ? '#2dd4bf' : 'var(--color-brand)' }}>{subtitle}</p>
      </div>
