...
const ShareButton = ({ name = '', title = '' }) => {
  ...
  const text = `Check out ${name}'s portfolio${title ? ` — ${title}` : ''}`;
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: `${name} — Portfolio`, text, url });
      } catch {}
    }
    setOpen(false);
  };
      ...
      {open && (
        <div className="absolute bottom-full left-0 right-0 mb-2 rounded-xl overflow-hidden shadow-2xl animate-slide-up"
          ...
          <div className="p-2 space-y-0.5">
            {platforms.map(p => (
              <a key={p.name} href={p.href} target="_blank" rel="noreferrer noopener"
            ...
            <button onClick={handleCopy}
              ...
              {copied
                ...
                : <><Copy size={16} style={{ color: 'var(--color-text-muted)' }} /> Copy Link</>
              }
            </button>
          </div>
        </div>
